...
let n_52 =
  let name = "n_52" in
  let params = [paramdef "i" "client"; paramdef "j" "client"] in
  let formula = (imply (andList [(andList [(neg (eqn (param (paramref "i")) (param (paramref "j")))); (neg (eqn (param (paramref "i")) (var (global "Home"))))]); (neg (eqn (param (paramref "j")) (var (global "Home"))))]) (imply (andList [(neg (eqn (var (arr [("n", [paramref "i"])])) (const _T))); (neg (eqn (var (arr [("n", [paramref "i"])])) (const _I)))]) (neg (eqn (var (arr [("n", [paramref "j"])])) (const _E))))) in
  prop name params formula

let n_c51 =
  let name = "n_c51" in
  let params = [] in
  let formula = (imply (neg (eqn (var (global "i")) (var (global "j")))) (imply (eqn (var (arr [("n", [paramref "j"])])) (const _C)) (neg (eqn (var (arr [("n", [paramref "i"])])) (const _E))))) in
  prop name params formula

let n_52 =
  let name = "n_52" in
  let params = [] in
  let formula = (andList [(andList [(neg (eqn (var (global "i")) (var (global "j")))); (neg (eqn (var (global "i")) (var (global "Home"))))]); (neg (eqn (var (global "j")) (var (global "Home"))))]) in
  prop name params formula  n_c51; n_52; n_c51; n_52 *)
