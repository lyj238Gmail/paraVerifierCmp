...
let preProcessAndProp p =
 ...
 (* let ()=print_endline name in
  ...
  |_ ->[Prop(name,params,Imply(Chaos,formula))]
...
let  () =  
  ...
  let ()=Cmp.setPrules rules in  
	let ()=Cmp.notConsiderTypeII:=true in
	Cmp.cmpGenChk  [global "CurPtr"] properties ~types:types  paraRef   [1;2] ~unAbstractedReqs:[pair] [] rules  protocol ["NODE";"DATA"] [global "CurPtr"]
