...
let printInvs invs =
	...
	~f:(fun (f,i) ->   
	(  (LoachGeneralize.concreteInv2ParamProp (Loach.Trans.invTrans_formula  f) i))) in
...
let prot= Loach.Trans.act ~loach:protocol in  
  ...
  let a=CheckInv.startServer ~murphi:(In_channel.read_all "n_flash.m")
    ~smv:(In_channel.read_all "mutualEx.smv") "n_flash"  "n_flash" 
    host host ~types:types ~vardefs:vardefs protocol in
  let ()=print_endline "---SetOfALL\n" in  
...
(*let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = find protocol
    ~murphi:(In_channel.read_all "n_flashChou_without_nxt_home_126nLyj.m")
  in
