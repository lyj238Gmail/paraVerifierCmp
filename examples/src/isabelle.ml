...
let var_act' (Arr(name_with_prs)) =
  ...
  | Boolc(b) -> sprintf "%b" b
  | Strc(s) -> s
  | Intc(i) -> sprintf "%d" (i - 1)  in

  let cast_to_string init prs =
    List.fold prs ~init ~f:(fun res x -> sprintf "(Para %s %s)" res (constIndexDec (index_of_param x))) 
  in
	try
	let ()=print_endline (ToMurphi.var_act (Arr(name_with_prs))) in 
  match name_with_prs with
  | [] -> raise Empty_exception
  | (name, prs)::name_with_prs' ->
    let ident = sprintf "(Ident ''%s'')" name in
	...
	with
	|Unexhausted_inst-> let ()=print_endline (ToMurphi.var_act (Arr(name_with_prs))) in raise Unexhausted_inst
    ...
    begin
      ...
      | [Paramdef(name, tname)] ->
        let form_str = formula_act form in				
				let ncOrN=if (!enterAbsRule) then "NC" else  "N"  in
        sprintf "(forallForm (down %s) (\\<lambda>%s. %s))" ncOrN name form_str
  ...
  | ExistFormula(paramdefs, form) ->
    begin
      ...
      | [Paramdef(name, tname)] ->
        let form_str = formula_act form in							
				let ncOrN=if (!enterAbsRule) then "NC" else  "N"  in
        sprintf "(existsForm (down %s) (\\<lambda>%s. %s))" ncOrN  name form_str
      ...
      begin
        ...
        | [Paramdef(name, tname)] ->
          ...
          let s_str = trans s in
					let ncOrN=if (!enterAbsRule) then "NC" else  "N"  in
          sprintf "(forallSent (down %s) (\\<lambda>%s. %s))" ncOrN name s_str
...
let rule_act r =
  ...
  let Rule(name, pds, f, s) = r in
	let tmp=enterAbsRule:=if (not (List.exists ~f:(fun x->x=name) !absRules)) then false else true in
  ...
  let quant = if (!quant_in_rule) && (not (List.exists ~f:(fun x->x=name) !absRules)) then "N" else "" in
  let quant_type = if (!quant_in_rule) && (not (List.exists ~f:(fun x->x=name) !absRules))  then "nat \\<Rightarrow> " else "" in
...
let invs_act_without_neg cinvs=
  let invs_with_pd_count = List.map cinvs ~f:inv_act_without_neg in
  let inv_strs = String.concat ~sep:"\n\n" (List.map invs_with_pd_count ~f:(fun (_, _, s) -> s)) in
  let inv_insts_str = String.concat ~sep:" \\<or>\n" (
    List.map cinvs ~f:(fun (ConcreteProp(Prop(name, pds, _), _)) ->
      if List.is_empty pds then
