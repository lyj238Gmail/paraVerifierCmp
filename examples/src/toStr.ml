  ...
  let var_act v =
    ...
    if params = [] then
      ...
      let actual_ps = List.map params ~f:(fun p ->
        ...
        | Paramref(n) -> n (*let ()=print_endline n in  raise Unexhausted_inst*)
      ) in
...
module Smv = struct
  ...
  let paramref_act pr =
    ...
    | Paramref(name) ->  sprintf "[%s]" (name)
	(*Duan's version:let ()=print_endline name in Prt.error ("Unexhausted_inst param: "^name);raise Unexhausted_inst
	*)
  ...
  let rec exp_act exp =
    ...
    | Param(pr) -> (
        match pr with
        | Paramfix(_, _, c) -> sprintf "%s" (const_act c)
        | Paramref(n) -> let ()=print_endline n in  raise Unexhausted_inst
      )
    | Ite(f, e1, e2) ->
