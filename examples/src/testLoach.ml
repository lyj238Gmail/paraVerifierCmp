...
let n_CacheStateProp =
  ...
  let formula = (  ( (imply (neg (eqn (param (paramref "p")) (param (paramref "q")))) (neg (andList [(eqn (var (record [global "Sta"; arr [("Proc", [paramref "p"])]; global "CacheState"])) (const _CACHE_E)); (eqn (var (record [global "Sta"; arr [("Proc", [paramref "q"])]; global "CacheState"])) (const _CACHE_E))]))))) in
...
let () =  
  let a=
	...
	let b=List.map ~f:(fun  r->print_endline (ToMurphi.rule_act r)) (CMP.instantiatePr2Rules  paraRef  [1;2] types n_NI_InvAck_exists) in
	let ()=print_endline "---\n" in
	let rules=CMP.cmp properties ~types:types  paraRef  [1;2] n_Store in (*cmp 	pprops ~types paramRef  nodes pr*)
	List.map ~f:(fun  r -> print_endline (ToMurphi.rule_act r)) rules in
	()
