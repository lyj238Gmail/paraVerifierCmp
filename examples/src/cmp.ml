...
>>>>>>> 7bf2ad7ffd983b8c2d373302b928ab4ee3d23d36
		...
		|Some(((absParams,absR),usedInvs))->
			let props=List.filter 
				~f:(fun p -> let Loach.Prop(pn,pds,pinv)=p in
					List.exists ~f:(fun (pn',inv)->pn'=pn) (usedInvs))
				(pprops)  in
			GenCmpProof.CaseAbs(List.map ~f:parafix2paradef absParams,  r, absR,props,[])
		|None->(let ()=print_endline "genSkip" in GenCmpProof.CaseSkip( [], r)) in
...
let isabelle2AbsInvs cinvs=
	let invAct cinv =
		let (InvFinder.ConcreteProp(Prop(name, pds, pinv), _)) =cinv in
			...
			then List.map ~f:(fun n->String.concat ~sep:" " [name;n]) ["0"]
...
let outAbsProtWithCmpLemmas ~types absProt cases=
	...
	let ruleStr=  Isabelle.rules_act absProt.rules in
...
"		in
	let initsStr=Isabelle.inits_act absProts.inits in
	let caseStr=GenCmpProof.lemmaGenerates cases in
	let filename =   absProt.name^"_abs.thy"   in 
	Unix.mkdir_p absProt.name;
	Isabelle.file_pub absProt.name typeStr (ncStr^ruleStr) (invStr^absInvStr^initsStr^axiomStr) (caseStr) ()
 ...
 (* let {name=name0; types=types0; vardefs=vardefs0; init=init0; rules=rules0; properties=properties0} :Loach.protocol= prot0   in*)
	...
	let absProt=	
		{name=prot0.name; types=prot0.types; vardefs=prot0.vardefs;
		 ...
		 properties=props}  in
	let ()=outAbsProtWithCmpLemmas ~types absProt cases in
	absProt
