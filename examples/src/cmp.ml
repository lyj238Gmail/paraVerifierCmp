...
open ToStr
	...
	let invsInsts=ref []
	let prulesRef= ref []
	let notConsiderTypeII=ref false
...
let rec  statementAllOn exVars index statement= 
	match statement with
	|Assign(var,value) ->  onlyOnVar index var (*|| (onExp index value && List.mem exVars var)*)
	|Parallel(statementList) -> 
		List.fold  (List.map ~f:( statementAllOn  exVars  index) statementList) 
		 ~f:(fun x y -> x && y) ~init:true
	|IfStatement(bcond,statement) ->
		 statementAllOn  exVars  index statement
	|IfelseStatement(b,s1,s2) ->
	 ( statementAllOn  exVars  index s1) && ( statementAllOn  exVars  index s2)
	|ForStatement(s,parmDefs) ->false
		(*regularForStatement(s)*)
		
let statementAllOnIndexs exVars absIndexs  st		=
	List.fold ~f:(fun  y index->statementAllOn  exVars index st || y) absIndexs ~init:false

let  onlyOnRule  exVars  index rule=
...
let propInstForRuleWith1Para pprops ~types paramRef=
	...
	(*let invs= if (!notConsiderTypeII) 
						then (List.filter invs 
							~f:(fun x-> 
									let (invn,prefs,invariant)=x in
	                let Imply(ant,cons)=invariant in
	                let antParams=Loach.ParasOf.of_form ant in
	                let consParams=Loach.ParasOf.of_form cons in
	                let generalizedParas=  Int.Set.diff consParams antParams in
									if ((Int.Set.is_empty generalizedParas) && not (Int.Set.is_empty consParams))
									then false
									else true))
						else invs in*)
...
let needCmp exVars absIndexs types notOtherExps r=	
	let Rule(name,paramdefs,g, act)=r  in
	(*let  Paramecium.Paramfix(pname,indextype,Intc(index))=paramRef in	*)
	let  statements=statementDecompse act in
	let statements=List.filter ~f:(fun st->not (statementAllOnIndexs exVars absIndexs  st ))  statements in
...
let rec consistIf s=
	match s with
    | Assign(v, e) -> false
  | Parallel(slist) -> List.fold   (List.map slist ~f:consistIf) ~f:(fun x y->x || y)     ~init:false
	  | IfStatement(cond,s)-> true
  | IfelseStatement(f,s1,s2) ->true
  | ForStatement(s,pdfs) -> consistIf s

let rec simplify_if_statement s=
	match s with
| Assign(v, e) -> s
    | Parallel(slist) -> Parallel(List.map slist ~f:(simplify_if_statement))    
|IfStatement(cond,s0) ->  IfStatement(eliminate_false_eq 3 [] cond, s0)
|IfelseStatement(f,s1,s2) -> IfelseStatement(eliminate_false_eq 3 [] f,s1,s2)
    			
let simpForStatemet absIndexs forallStatement=
	let ForStatement(s, pd)=forallStatement in 
	if not(consistIf s) then forallStatement
	else 
	begin 
		let pSub=List.map ~f:(fun p -> let Paramecium.Paramdef(n,t)=p in Paramecium.Paramfix(n,t,Intc(1))) pd in
		let sInst=apply_statement_without_fold_forStatement ~p:pSub ~types:[] s in
		let s'=simplify_if_statement sInst in
		let generalizedParas=Int.Set.diff  (ParasOf.of_statement sInst) (Int.Set.of_list absIndexs) in
		let (pds,prs,ps)=LoachGeneralize.statement_act  
		 	~generalizedtParas:( Paramecium.int_consts (Int.Set.to_list generalizedParas)) ~rename:false s' [] [] in
		ForStatement(ps, pds)
	end

let rec simpFor  absIndexs s=
	match s with
    | Assign(v, e) -> s
  | Parallel(slist) ->    Parallel(List.map slist ~f:(simpFor absIndexs))  
	  | IfStatement(cond,s')-> s
  | IfelseStatement(f,s1,s2) ->s
  | ForStatement(s',pdfs) -> simpForStatemet absIndexs s
...
let cmpAbstract exVars absIndexs types r=
	let Rule(name,paramdefs,g, act)=r  in
	...
	(pnsOnAbstractInd,Rule(name',paramdefs,g, simpFor  absIndexs s))
	...
	then raise OtherException
		...
		then
			...
			let para= Paramecium.Paramfix(pn,pt,Intc(index)) in
			   [(Loach.apply_rule_without_fold_forStatement  ~p:[para] ~types:types pr, [index;index+1])]  
...
let cmp exVars	 pprops ~types paramRef  nodes ?(unAbstractedParas=[])  notOtherExps  pr=
	let  Paramecium.Paramfix(pname,indextype,Intc(index))=paramRef in	
	let  Loach.Rule(name,paramdefs,g, act)=pr in	
	...
	let rs1=List.filter ~f:(fun (r,abs_is)-> not (needCmp exVars abs_is types  notOtherExps r ) ) rs in
	let rs1'=List.map 		~f:(fun  (r,abs_is) ->(None, abs_is)) rs1 in 
  (*rs1' is those which are not need to be cmped*)
	let rs=List.filter ~f:(fun (r,abs_is)-> needCmp exVars abs_is types  notOtherExps r  ) rs in
	match rs with
	|[] ->( rs1',[])
	|_ ->
 ...
 (*cmpStrengthRule invs  ~types paramRef r*)
	...
	let rs'=List.map 
		~f:(fun  x ->match x with 
		 |(Some(r,usedInvs),abs_is) ->(Some(cmpAbstract exVars abs_is types r,usedInvs),abs_is)
...
let cmpOnPrs 	exVars pprops ~types paramRef  nodes  ?(unAbstractedReqs=[])  notOtherExps  prs=
	let cmpt r=
		...
		let ls=List.filter ~f:(fun (prName,paras) -> name=prName) unAbstractedReqs in
			...
			|_ ->let Some(pair)=List.hd ls in
				snd pair  in
	let resultL=List.map ~f:(fun r -> cmp exVars	pprops ~types paramRef  nodes ~unAbstractedParas:(cmpt r) notOtherExps r) prs in
...
let cmpAbsProtGen  exVars pprops ~types paramRef  nodes  ?(unAbstractedReqs=[])  notOtherExps  prs (prot0:Loach.protocol) varLsInVF'=

	let result=cmpOnPrs  exVars 	pprops ~types paramRef  nodes  ~unAbstractedReqs  notOtherExps  prs in
...
let cmpGenChk exVars pprops ~types paramRef  nodes  ?(unAbstractedReqs=[])  notOtherExps  prs (prot0:Loach.protocol) scalars varLsInVF'=
	let absProt=cmpAbsProtGen exVars pprops ~types paramRef  nodes  ~unAbstractedReqs notOtherExps  prs (prot0:Loach.protocol) varLsInVF' in
	let tmp=ToMurphi.setScalarTypes scalars in
	let tmp=ToMurphi.setOther paramRef in
