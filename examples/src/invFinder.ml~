...
let check_invs_on_init_byForm cinvs init =
  let assigns = statement_2_assigns init in
  List.map cinvs ~f:(fun cinv ->
    let inv =  cinv in
    cinv, List.filter assigns ~f:(fun assign ->
...
module Choose = struct
  ...
  let check_level ?(must_new=false) init inv =
        ...
        | None ->
          ...
          let f=if (!symmetry_method_switch) then 
            ...
            not_inv
    			...
    			try 
    			let a=(check_invs_on_init_byForm [neg f] init)   in			
    			
          if must_new || Smv.is_inv (ToStr.Smv.form_act f) then
            new_inv inv
          else begin not_inv   end
          with
          | _->(let ()=print_endline "kk" in not_inv)
  ...
  (* choose new inv *)
  let choose init guards assigns cons =
    check_level ~must_new:true init (simplify (andList (cons::guards)))
...
let deal_with_case_3 init crule cinv cons g =
  let Rule(name, _, guard, statement), _, guards, assigns = concrete_rule_2_rule_inst crule in
  let level = Choose.choose init (concrete_prop_2_form g::guards) assigns cons in
...
let tabular_expans init (Rule(_name, _, form, _), crule, _, assigns) ~cinv =
    ...
    | (g, obligation)::obligations' ->
      ...
      let relation = 
        ...
        (* case 3 *)
          ...
          deal_with_case_3 init crule cinv obligation branch
        end
      in
...
let get_res_of_cinv init cinv rname_paraminfo_pairs =
  ...
  let new_invs, new_relations =
    let invs, rels = List.unzip (List.map crules ~f:(fun r_insts ->
      List. unzip (List.map r_insts ~f:(fun r_inst ->
        List.unzip (tabular_expans init r_inst ~cinv)
      ))
    )) in
...
let tabular_rules_cinvs init rname_paraminfo_pairs cinvs relations =
  let rec wrapper cinvs relations =
    match cinvs with
    | [] -> (InvLib.get_all_cinvs (), relations)
    | cinv::cinvs' ->
      let (new_cinvs, new_relations) = get_res_of_cinv init cinv rname_paraminfo_pairs in
...
let find ?(insym_types=[]) ?(smv_escape=(fun inv_str -> inv_str))
  ...
  let _smv_context =
  	let ()=Smv.protocol_name:=name in 0 
  ...
  let ()=print_endline "enter compute invs" in
  let (cinvs, relations) = tabular_rules_cinvs init rname_paraminfo_pairs cinvs relations in
  ...
  let b=List.map ~f:(fun cp ->
  	let ConcreteProp(ccp,_) =cp in
  	let lcp=Loach.Trans.invTrans_prop ccp in
  	 print_endline ( ToMurphi.prop_act lcp)) cinvs in
