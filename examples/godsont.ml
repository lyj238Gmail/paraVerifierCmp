...
(*
let n_DeadlockFree1 =
  let name = "n_DeadlockFree1" in
  let params = [paramdef "i" "TYPE_NODE"] in
  let formula = (neg (andList [(andList [(eqn (var (global "replace")) (const _NON)); (eqn (var (record [arr [("node", [paramref "i"])]; global "hasLock"])) (const _True))]); (forallFormula [paramdef "l" "TYPE_LOCK"] (orList [(eqn (var (record [arr [("lock", [paramref "l"])]; global "beUsed"])) (const _False)); (neg (eqn (var (record [arr [("lock", [paramref "l"])]; global "owner"])) (param (paramref "i"))))]))])) in
