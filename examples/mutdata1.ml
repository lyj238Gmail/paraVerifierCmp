...
let rules = [n_Try; n_Crit; n_Exit; n_Idle; n_Store]
...
let n_concreteCoherence =
  let name = "n_concreteCoherence" in
  let params = [] in
  let formula = (imply (eqn (var (record [arr [("n", [paramref "1"])]; global "st"])) (const _C)) (neg (eqn (var (record [arr [("n", [paramref "2"])]; global "st"])) (const _C)))) in
  prop name params formula
...
let properties = [n_coherence; n_dataprop; n_concreteCoherence; n_dataprop]
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = find protocol
    ~murphi:(In_channel.read_all "n_mutdata1.m")
  in
