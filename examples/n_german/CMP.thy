...
(*assumes "\<forall>r. r \<in> rs \<longrightarrow> r \<in> rs' \<or> (\<exists>inv\<in>Inv. strengthenRule inv r \<in> rs')"*)
...
lemma strengthenProt1SimProt:
  assumes a1:"\<forall>r. r \<in> rs \<longrightarrow>((\<exists> f . f \<in>  S \<and> strengthenRule f r \<in> rs') | r \<in> rs')" and
  a2:"\<forall>i s f. reachableUpTo I rs' i s \<longrightarrow> f \<in> S \<longrightarrow>formEval f s" 
...
next
  ...
  proof((rule allI)+,(rule impI)+)
    ...
    have " 
        (\<exists>s0  g a.      reachableUpTo I rs n s0\<and> formEval g s0 \<and> trans1 a s0=s \<and> guard g a \<in>rs) "
      by (metis b1 reachableUpToSuc)
       
    then  obtain s0   g a where c1:"     reachableUpTo I rs n s0\<and> formEval g s0 \<and>
     trans1 a s0=s \<and> guard g a \<in>rs"
      by blast
    let ?r="guard g a"
    ...
    ultimately show "reachableUpTo I rs' (Suc n) s \<and> (\<forall>f. f \<in> S \<longrightarrow> formEval f s)"
      using \<open>f \<in> S \<and> strengthenRule f (g \<triangleright> a) \<in> rs' \<Longrightarrow> reachableUpTo I rs' (Suc n) s \<and> (\<forall>f. f \<in> S \<longrightarrow> formEval f s)\<close> by blast
...
lemma strengthenProt2SimProt1:
  assumes a1:"\<forall>r1. r1 \<in> rs1 \<longrightarrow>((\<exists>r f. r\<in>rs \<and>f\<in>  S \<and> r1= strengthenRule f  r 
  \<and>  strengthenRule2 f  r \<in> rs2 ) |(  r1 \<in> rs2))" and
  a2:"\<forall>i s f. reachableUpTo I rs2 i s\<longrightarrow> f \<in> S \<longrightarrow>formEval f s" 
...
next
  ...
  proof((rule allI)+,(rule impI)+)
    ...
    have c1:"
        (\<exists>s0 g a. guard g a \<in>rs1 \<and>   reachableUpTo I rs1 n s0\<and> formEval g s0 \<and> trans1 a s0=s) "
      by (metis b1 reachableUpToSuc) 
    from c1 obtain s0 g a  where c1:"guard g a \<in>rs1 \<and>   reachableUpTo I rs1 n s0\<and> 
      formEval g s0 \<and> trans1 a s0=s"
        by blast
      have c2:"(\<exists>r f. f \<in>  S \<and> r\<in>rs \<and>  (guard g a)=strengthenRule f r 
  \<and> strengthenRule2 f r \<in> rs2)| (   (guard g a) \<in> rs2)"
        using a1 c1 by blast
(*" (\<exists> Ls ss. set Ls \<subseteq> set S\<and>  set ss \<subseteq> set S  \<and>  strengthenR1 Ls ss r \<in> rs') "*)
        
      moreover
      {assume c2:"(\<exists>r f. f \<in>  S \<and> r\<in>rs \<and>  (guard g a)=strengthenRule f r 
  \<and> strengthenRule2 f r \<in> rs2)"
      from c2 obtain r f where c2:"f \<in>  S\<and> r\<in>rs \<and>  (guard g a)=strengthenRule f r 
  \<and> strengthenRule2 f r \<in> rs2"
        by blast
      from b0 c1 c2 have c3:"\<forall>f. f \<in> S \<longrightarrow> formEval f s0"
        by auto
...
lemma strengthenProt2SimProt:
  assumes a1:"\<forall>r. r \<in> rs \<longrightarrow>((\<exists> f. f\<in>  S \<and>  strengthenRule2 f  r \<in>rs2
   ) |( r \<in> rs2  ))" and
  a2:"\<forall>i s f. reachableUpTo I rs2 i s\<longrightarrow> f \<in> S \<longrightarrow>formEval f s" 
shows "\<forall>s . reachableUpTo I rs i s\<longrightarrow>
   (reachableUpTo I rs2 i s\<and>(\<forall>f. f \<in> S \<longrightarrow>formEval f s))" (is "?P i")
proof(rule allI, rule impI)
 ...
 reachableUpTo I rs2 i s\<and>(\<forall>f. f \<in> S \<longrightarrow>formEval f s)" thm strengthenProt2SimProt1
  proof(rule allI,rule_tac ?rs1.0="?rs1" and ?rs="rs" and ?rs2.0="rs2" in strengthenProt2SimProt1
      ,auto,cut_tac a2,blast)
 qed

 have b3:"\<forall>s . reachableUpTo I rs i s\<longrightarrow>
 reachableUpTo I ?rs1 i s\<and>(\<forall>f. f \<in> S \<longrightarrow>formEval f s)"  thm strengthenProtSim 
 proof(rule_tac ?rs'="?rs1" and ?rs="rs"   in strengthenProt1SimProt)
   show "\<forall>r. r \<in> rs \<longrightarrow>
        (\<exists>f. f \<in> S \<and>
             strengthenRule f r
             \<in> {r'. (\<exists>f r. f \<in> S \<and> r \<in> rs \<and> r' = strengthenRule f r \<and> strengthenRule2 f r \<in> rs2) \<or>
                    r'\<in>rs\<and> r' \<in> rs2}) \<or>
        r \<in> {r'. (\<exists>f r. f \<in> S \<and> r \<in> rs \<and> r' = strengthenRule f r \<and> strengthenRule2 f r \<in> rs2) \<or>
       r'\<in>rs\<and>r' \<in> rs2}"
     using a1 by auto
 next
   show " \<forall>i s f.
       reachableUpTo I
        ?rs1
        i s \<longrightarrow>
       f \<in> S \<longrightarrow> formEval f s"
     using b2 by blast
 qed    
...
(*a1:"(((\<forall> v. v\<in> varOfExp e \<longrightarrow>v\<noteq>dontCareVar\<and> (v = absTransfVar M v ))\<and>safeIndexExp M e \<and> e\<noteq>dontCareExp)
        \<longrightarrow> e\<noteq>dontCareExp\<and>e = absTransfExp M e)\<and> 
  (((\<forall> v. v\<in> varOfForm f \<longrightarrow> v\<noteq>dontCareVar\<and>(v = absTransfVar M v )) \<and> safeIndexForm M f \<and>f\<noteq>dontCareForm)
  \<longrightarrow>f\<noteq>dontCareForm\<and> f = absTransfForm M f )  "
(is "?Pe e \<and> ?Pf f")
(* proof(induction rule: expType_formula.induct) *)
proof(induct_tac e and f,force,case_tac x,force,force,force,force )
  fix f e1 e2 
  assume b1:"?Pf f" and
  b2:"?Pe e1" and b3:"?Pe e2"
  show "?Pe (iteForm f e1 e2 )"
  proof(rule impI)
    assume c0:"(\<forall>v. v \<in> varOfExp (iteForm f e1 e2) \<longrightarrow> v \<noteq> dontCareVar \<and> v = absTransfVar M v) \<and>
    safeIndexExp M (iteForm f e1 e2)"
    have c1:"((\<forall> v. v\<in> varOfExp e1 \<longrightarrow>v\<noteq>dontCareVar\<and> (v = absTransfVar M v ))\<and>safeIndexExp M e1
       \<and> e1\<noteq>dontCareExp)"
      by(cut_tac c0, simp)
    have c2:"((\<forall> v. v\<in> varOfExp e2 \<longrightarrow>v\<noteq>dontCareVar\<and> (v = absTransfVar M v ))\<and>safeIndexExp M e2)"
      by(cut_tac c0, simp)
    have c3:"((\<forall> v. v\<in> varOfForm f \<longrightarrow> v\<noteq>dontCareVar\<and>(v = absTransfVar M v )) \<and> safeIndexForm M f)"
      by(cut_tac c0, simp)
    have c4:" e1 = absTransfExp M e1"
      using b2 c1 by blast
    have c5:" e2 = absTransfExp M e2"
      using b3 c2 by blast  
     have c6:" f = absTransfForm M f"
       using b1 c3 by blast
     show "iteForm f e1 e2 = absTransfExp M (iteForm f e1 e2) "
...
lemma absDontAffect:
  assumes  
  a2:"(\<forall>v. v\<in>varOfForm f\<longrightarrow> (s v= (abs1 M s) v))"
  shows "formEval f s = formEval f (abs1 M s)"
  by (simp add: a2 dontAffect)
   

lemma strengthenRule2Keep:
  assumes     a2:" wellFormedRule N r"
...
definition symRules :: "nat \<Rightarrow> (rule set) \<Rightarrow> bool" where
  "symRules N rs =
    (\<forall>r p. r\<in>rs \<longrightarrow>p permutes {x. x \<le> N}  \<longrightarrow> (\<exists>r'. r'\<in>rs\<and> equivRule (applySym2Rule p r ) r'))"

(*lemma symStrengthenRule2:
  assumes a1:"symProtRules N rs" and
  a2:"symPredSet N fs"
shows "symProtRules N {r'. (EX  r f. f \<in>fs \<and> r\<in>rs \<and> r'=strengthenRule2 f r) |r'\<in> rs}"
*)  


    
definition oneParamCons::"nat \<Rightarrow>(nat => rule) => rule set" where [simp]:
...
definition oneParamFormCons::"nat \<Rightarrow>(nat => formula) => formula set" where
"oneParamFormCons N pr \<equiv>{r. \<exists>i. i\<le> N \<and> r = pr i}"

definition oneParamFormCons2::"nat \<Rightarrow>(nat =>nat\<Rightarrow> formula) => formula set" where
...
proof((rule allI)+,(rule impI)+)
  ...
  then obtain p where b1:"p permutes {i. i \<le> N} \<and> p 0 = i \<and> p 1 = j"
    by blast
  show " \<exists>p. p permutes {i. i \<le> N} \<and>
   equivForm (applySym2Form p (constrInv pair 0 1 )) (constrInv pair i j)"
  proof(rule_tac x="p" in exI)
...
lemma strengthensymPair:
  assumes a1:"symPair  p N" and "symParamRule N r"
  shows "symParamRule N  (%i. (strengthenRule2  (constrInvByExcl p i N) (r i)))"
...
next
  ...
  show "symParamForm N (\<lambda>i. constrInvByExcl p i N) "   thm symParamFormForallExcl
...
proof(simp,rule+)
  fix j
  assume b1:"formEval (fst pair i) s " and
  b2:"  j \<noteq> i" and b3:"j \<le> N "
  have c1:"(\<exists>p. p permutes {i. i \<le> N}\<and> 
    equivForm (applySym2Form p (constrInv pair 0 1)) (constrInv pair i j) )"
    using assms(3) assms(4) assms(7) b2 b3 symOnFuncInv by blast
  then obtain p where c1:"p permutes {i. i \<le> N}\<and> 
    equivForm (applySym2Form p (constrInv pair 0 1)) (constrInv pair i j)" 
    by blast
  have c2:"formEval (applySym2Form p (constrInv pair 0 1)) s"
    using SymLemma' assms(1) assms(2) assms(5) assms(6) c1 by blast
...
proof((rule allI)+,(rule impI)+)
 ...
 (* have b4:"\<exists>p f0. f0 \<in> F \<and> p permutes {i. i\<le>N } \<and> equivForm   f   (applySym2Form p f0)"
  ...
  proof((rule allI)+,(rule impI)+)
    ...
    show "formEval  f s"
      apply(cut_tac c5,simp only:c5) thm SymLemmaOnExcl
