...
let n_CacheStateProp =
  ...
  let params = [paramdef "p" "NODE"; paramdef "q" "NODE"] in
  let formula =  (imply  (eqn (var (record [global "Sta"; arr [("Proc", [paramref "p"])]; global "CacheState"])) (const _CACHE_E)) (neg (eqn (var (record [global "Sta"; arr [("Proc", [paramref "q"])]; global "CacheState"])) (const _CACHE_E)))) in
...
let () =  
	let cmp1=(n_MemDataProp =n_CacheStatePropHome )in
	let ()=if cmp1 then print_endline "true" else print_endline "false" in
	let cmp1=String.compare "1" "1" in
	let ()=if (cmp1=0) then print_endline "true" else print_endline "false" in
  let a=
	(*let [paramdef "src" "NODE"; paramdef "pp" "NODE"] *)
	...
	let b=List.map ~f:(fun  r->print_endline (ToMurphi.rule_act r)) (CMP.instantiatePr2Rules  paraRef  [1;2] types n_Store) in
	let ()=print_endline "---refine1\n" in
	let dparaDef=paramdef "data" "DATA" in
	let rules=CMP.cmp properties ~types:types  paraRef  [1;2] n_Store in (*cmp 	pprops ~types paramRef  nodes pr*)
	let a=List.map ~f:(fun  r -> print_endline (ToMurphi.rule_act r)) (fst rules) in
	let ()=print_endline "---refine2\n" in
	let rules=CMP.cmp properties ~types:types  paraRef  [1;2] ~unAbstractedParas:[dparaDef] n_Store in
	List.map ~f:(fun  r -> print_endline (ToMurphi.rule_act r)) (fst rules) in
	()
